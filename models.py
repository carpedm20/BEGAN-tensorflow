...
def GeneratorCNN(z, hidden_num, output_num, repeat_num, data_format, reuse):
    with tf.variable_scope("G", reuse=reuse) as vs:
        x = slim.fully_connected(z, np.prod([8, 8, hidden_num]))
        x = reshape(x, 8, 8, hidden_num, data_format)
        
        for idx in range(repeat_num):
            x = tf.nn.elu(slim.conv2d(x, hidden_num, 3, 1, data_format=data_format))
            x = tf.nn.elu(slim.conv2d(x, hidden_num, 3, 1, data_format=data_format))
            if idx < repeat_num - 1:
                x = upscale(x, 2, data_format)

        out = slim.conv2d(x, 3, 3, 1, data_format=data_format)
        #out = tf.nn.tanh(out)
...
def DiscriminatorCNN(x, input_channel, z_num, repeat_num, hidden_num, data_format):
    with tf.variable_scope("D") as vs:
        ...
        for idx in range(repeat_num):
            ...
            x = tf.nn.elu(slim.conv2d(x, channel_num, 3, 1, data_format=data_format))
            x = tf.nn.elu(slim.conv2d(x, channel_num, 3, 1, data_format=data_format))
            if idx < repeat_num - 1:
